...
- **Migration off Deprecated `androidx.security.crypto` (Phases 1–5)**:
  ...
  - **Phase 2 (`DatabasePassphraseManager`)**: Migrated the core database passphrase storage from `EncryptedSharedPreferences` to standard private `SharedPreferences` with field-level encryption via `KeystoreCipherProvider` using the dedicated alias `"db_passphrase_key"`. Included automatic, transparent one-time in-place migration to decrypt and re-encrypt existing credentials from legacy `database_passphrase_prefs.xml`, safely deleting the old file upon success.
  - **Phase 3 (`SecureVaultPassphraseManager`)**: Migrated SecureVault's isolated database passphrase storage from `EncryptedSharedPreferences` to private preferences encrypted via `KeystoreCipherProvider` using the isolated alias `"securevault_db_passphrase_key"`. Preserved fallback safety nets and added automatic one-time migration from `securevault_encrypted_prefs.xml`.
  - **Phase 4 (`SecureVaultMetadataSafety`)**: Replaced `EncryptedFile` and `MasterKey` with direct cipher-wrapped file I/O using `KeystoreCipherProvider` under the dedicated alias `"securevault_shadow_key"`. Includes transparent migration that decrypts existing `securevault_meta_shadow.json` snapshots via the old `EncryptedFile` API and rewrites them using the new scheme without interrupting data integrity verification or self-healing.
  - **Phase 5 (`SecureVaultBiometricTracker`)**: Migrated Master PIN/Passphrase credentials and lockout rate-limiting state from `EncryptedSharedPreferences` to private preferences with field-level encryption via `KeystoreCipherProvider` under the isolated alias `"securevault_credential_key"`. Fully retained PBKDF2 (210,000 iterations), constant-time verification, and exponential backoff lockout logic while providing automatic one-time migration for existing credentials and lockout states.
- **Backup & Extraction Rules**: Updated `data_extraction_rules.xml` to exclude all newly introduced preference storage files (`database_passphrase_prefs.xml`, `database_passphrase_fallback_prefs.xml`, `securevault_database_passphrase_prefs.xml`, `securevault_database_passphrase_fallback_prefs.xml`, `securevault_system_integrity_prefs.xml`, and `securevault_system_integrity_fallback_prefs.xml`) from both cloud backups and device-to-device transfers.
