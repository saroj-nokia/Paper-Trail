...
- **Migration off Deprecated `androidx.security.crypto` (Phases 1–5)**:
  ...
  - **Phase 3 (`SecureVaultPassphraseManager`)**: Migrated SecureVault's isolated database passphrase storage from `EncryptedSharedPreferences` to private preferences encrypted via `KeystoreCipherProvider` using the isolated alias `"securevault_db_passphrase_key"`.
  - **Phase 4 (`SecureVaultMetadataSafety`)**: Replaced `EncryptedFile` and `MasterKey` with direct cipher-wrapped file I/O using `KeystoreCipherProvider` under the dedicated alias `"securevault_shadow_key"`.
  - **Phase 5 (`SecureVaultBiometricTracker`)**: Migrated Master PIN/Passphrase credentials and lockout rate-limiting state from `EncryptedSharedPreferences` to private preferences with field-level encryption via `KeystoreCipherProvider` under the isolated alias `"securevault_credential_key"`. Retains PBKDF2 (210,000 iterations), constant-time verification, and exponential backoff lockout logic.
- **Complete Removal of Deprecated `androidx.security:security-crypto`**: Completely removed the deprecated dependency and deleted all legacy migration code branches that existed solely for pre-release data format conversions.
- **Elimination of Insecure Plaintext Fallbacks & Keystore Failure Gating**:
  - Removed all silent plaintext storage fallback code paths (`PREFS_FALLBACK_FILE`, `KEY_FALLBACK_PASSPHRASE`, `isFallbackMode`) across `DatabasePassphraseManager`, `SecureVaultPassphraseManager`, and `SecureVaultBiometricTracker`.
  - Introduced `KeystoreUnavailableException` and global security gating via `KeystoreSecurityState` and `KeystoreUnavailableScreen`. When hardware security (StrongBox/TEE) is unavailable, the application strictly refuses to store credentials unencrypted and surfaces a full-screen blocking UI with retry support.
  - Hardened `SecureVaultViewModel` and `SecureVaultScreen` to isolate and surface hardware Keystore unavailability safely without crashing or exposing unencrypted data.
- **Backup & Extraction Rules**: Updated `data_extraction_rules.xml` to exclude private preference storage files (`papertrail_database_passphrase_prefs.xml`, `securevault_database_passphrase_prefs.xml`, and `securevault_system_integrity_prefs.xml`) from both cloud backups and device-to-device transfers.
